...
//
// [20, 37, 37, 20, 21]  1nth => [20, 37, 21]
// [11, 11, 40, 11, 32, 11, 71, 40]  2nth => [11, 11, 40, 32, 71, 40]
//
...
console.log(
	'deleteNth:', deleteNth([11,11,40,11,32,11,71,40], 2)
);


//
// My Answer that didn't work.
...
function countAndSay1(n, result='1') {
	...
	for (var i=1; i <= result.length; i++) {
		if (result.charAt(i) == result.charAt(i-1)) {
			...
			buffer += count;
			buffer += result.charAt(i-1);
			count = 1;
		}
	}

	return countAndSay1(n-1, buffer);
}
...
function lookAndSay(number, position=0)
{
  ...
  return firstChar == number.charAt(position) ?
			...
			number.substring(0, position).length + firstChar + lookAndSay(number.substring(position), 0);
}
console.log(lookAndSay("1"));
console.log(lookAndSay("11"));
console.log(lookAndSay("21", 0));
console.log(lookAndSay("1211", 0));
console.log(lookAndSay("111221", 0));
// End of Groupon Coding Interview.
//


 function palindrome(str) {
  // Good luck!
  var orgin_str = str.replace(/[^a-zA-Z0-9]+/g, '').toLowerCase();
  var ret_str = orgin_str.split('').reverse().join('');
	console.log(`orgin_str:${orgin_str} ret_str:${ret_str}`);
  if (orgin_str === ret_str) return true;
  else return false;
}
...
function findLongestWord(str) {
  ...
  return parseInt(ret.pop().item);
}
console.log(
 'longest word length:', findLongestWord("The quick brown fox jumped over the lazy dog")
);
...
function titleCase(str) {
  ...
  var ret = [];
  arr.forEach(function(item) {
    var _ret = item.charAt(0).toUpperCase() + item.toLowerCase().slice(1);
    ret.push(_ret);
  });
  return ret.join(' ');
}
